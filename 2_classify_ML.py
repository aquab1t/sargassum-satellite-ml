...
try:
    ...
    pass # XGBoost availability checked where needed
...
# --- MODIFIED: List of models to run (Only XGBoost) ---
MODELS_TO_RUN = [
    ...
    try:
        ...
        with rasterio.open(output_path, 'w', **profile) as dst:
            ...
            for _, window in tqdm(windows_to_process, desc=f"  Classifying {scene_id} ({model_name_prefix})", unit="block", leave=False, mininterval=1.0):
                ...
                # --- MODIFIED: GPU Prediction Block for XGBoost ---
                try:
                    ...
                    # XGBoost (with device='cuda') can accept NumPy arrays
                    # and will handle moving data to the GPU.
                    input_data = features_for_model_scaled.astype(np.float32)

                    # Run GPU-accelerated prediction
                    pred_p_all_classes = model_obj.predict_proba(input_data)
...
# --- NEW: Function to load GPU-accelerated models ---
def load_gpu_model(model_path):
    model, actual_model_type = None, None
    ...
    try:
        if model_path.endswith('.joblib'):
            ...
            if XGB_AVAILABLE and isinstance(model, xgb.XGBClassifier):
                ...
                # Configure GPU inference once at load time instead of per-block
                model.set_params(device='cuda')
            else:
                print(f"Warning: Loaded joblib model {model_path} is not a recognized XGBClassifier: {type(model)}")
                return None, None
            print(f"  Successfully loaded Joblib model: {model_path} (Detected Type: {actual_model_type})")
        else:
            print(f"Error: Unknown model file extension for {model_path}. Expected .joblib")
            return None, None
    except Exception as e:
        print(f"Error loading model {model_path}: {e}"); traceback.print_exc()
        return None, None
...
if __name__ == "__main__":
    print("--- Starting GPU Classification (XGBoost ONLY) ---")
    ...
    if not XGB_AVAILABLE:
        print("CRITICAL: XGBoost not available. This script requires XGBoost. Exiting.")
        exit(1)
    ...
    for model_filename in MODELS_TO_RUN:
        ...
        model_obj, model_type = load_gpu_model(model_full_path)

        if model_obj and model_type:
    ...
    if not loaded_models_info:
        print("Error: No GPU-accelerated models were successfully loaded. Exiting.")
        exit(1)

    print(f"\nSuccessfully loaded {len(loaded_models_info)} XGBoost model(s). Starting classification...")
